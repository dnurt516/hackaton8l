...
const app = async() => {
    ...
    $('.weather--today__data .feel_temp').text(weather.wind.speed + " м/с" + " " + directionOfwWind(weather.wind.deg));
    ...
    for (var i = 0; i < forecast.list.length; i++) {
        forecast_days[getDate(forecast.list[i].dt, true)] = [];
    }

    for (var i = 0; i < forecast.list.length; i++) {
        forecast_days[getDate(forecast.list[i].dt, true)].push(forecast.list[i].main.feels_like);
        forecast_days[getDate(forecast.list[i].dt, true)].push(forecast.list[i].main.temp);
    }
